...
class CrawlJob < ApplicationJob
  ...
  def perform(crawl_run_id:, options: {})
    ...
    if geocode_results.empty?
      ...
      # Nothing more can be done without coordinates — stop here.
      return
    end
    ...
    # `false` object. Flagged separately below as worth double-checking.
    ...
    #
    # Plain local variables that will accumulate running totals across all
    # companies crawled — note these get updated from MULTIPLE THREADS
    # later, which is exactly why the Mutex further down exists.
    total_facilities = 0
    total_units      = 0
    ...
    Playwright.create(playwright_cli_executable_path: playwright_path) do |playwright|
      ...
      # -----------------------------------------------------------------------
      #
      # This section is the heart of the file's concurrency (multiple
      # things happening "at once") logic. Below is the full setup before
      # any threads actually start.
      ...
      # variable (like `total_facilities` above, or the work queue built
      # next), only one thread does so at a time, preventing corrupted
      # updates.
      ...
      # still needed elsewhere, e.g. already saved to crawl_run above).
      ...
      max_parallel.times do
        ...
        thread = Thread.new do
          ...
          loop do
            ...
            # `begin ... rescue ... end` wraps the actual per-company
            # crawl attempt so that ANY failure for ONE company (a crash,
            # a timeout, a bad parser) is caught here and doesn't kill this
            # worker thread — the thread just logs the failure and loops
            # around to grab the NEXT company instead.
            begin
              broadcast_log(
              ...
              # between 1 second (minimum, so Timeout.timeout below is never
              # called with 0 or a negative number, which would be invalid)
              # and COMPANY_TIMEOUT_SECONDS (maximum, redundant with the
              # .min above but defensive).
              ...
              # Ruby can't interrupt (mentioned again near the `.join`
              # call further down) can still block past this timeout.
              ...
              #
              # Again using `mutex.synchronize do ... end` because
              # `total_facilities`/`total_units` are ordinary local
              # variables shared across ALL worker threads — without the
              # mutex, two threads finishing at nearly the same moment
              # could both read the same starting value and one thread's
              # update could get silently overwritten/lost (the classic
              # "race condition" explained at the top of this file).
              mutex.synchronize do
                # `.to_i` defensively converts whatever came back (in case
                # it's nil or already a different numeric type) to a plain
              ...
              # multiple threads WITHOUT needing the mutex (unlike the
              # plain Ruby variables above).
              ...
              # detail as reasonably fits, then lets the loop continue to
              # the next company rather than crashing this whole worker
              # thread.
              crawl_run.log_error(
            ...
            # `end` closes the `begin/rescue` block for this one company —
            # after this, the `loop do` above continues around to try
            # claiming the NEXT company from the shared queue.
          end
      ...
      active_threads.select(&:alive?).each do |t|
        ...
        # (inside the thread) apparently failed to interrupt whatever
        # native/blocking call it was stuck in, and even the grace-period
        # join above timed out waiting for it. This is the last-resort
        # backstop mentioned in the comment above.
        crawl_run.log_warning(
      ...
      # truly finished, before moving on to the remaining steps below.
      browser.close
    end # Playwright.create block ends here — browser is automatically closed
    # This trailing `end` closes the `Playwright.create(...) do
    # |playwright|` block that began well above (back at STEP 3) — every
    # line of STEP 3, 4, and 5 happened inside this block, using the
    # `playwright`/`browser` objects it provided.
    ...
    #
    # Catch-all for any other unexpected error anywhere in `perform` that
    # wasn't already handled by a more specific rescue (like the
    # per-company rescues inside the thread blocks above, which don't let
    # errors escape this far).
  ...
  # `end` closes the `def perform` method definition — this is the end of
  # the entire multi-step job logic described at the top of the file.
